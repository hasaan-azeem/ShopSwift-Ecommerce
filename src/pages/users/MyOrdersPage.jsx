    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-red-500 text-center px-4">
        <div>
          ...
          <Link
            to="/login"
            className="mt-4 inline-block bg-black text-white px-6 py-2 rounded-lg text-sm"
          >
            Login
          </Link>
        </div>
        ...
        {orders.map((order) => (
          ...
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[order.status] || "bg-gray-100 text-gray-600"}`}
              >
                {order.status}
              </span>
            </div>
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Qty: {item.qty} × PKR {item.price?.toLocaleString()}
                    </p>
                  </div>
                </div>
              ))}
