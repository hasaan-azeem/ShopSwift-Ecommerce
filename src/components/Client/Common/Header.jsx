    ...
    <>
      <header className="w-full bg-white fixed top-0 left-0 border-b border-gray-200 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between py-3 px-4 md:px-6">
          <Link to="/" className="text-2xl font-semibold tracking-wide text-gray-900">ShopSwift</Link>

          <nav className="hidden md:flex space-x-10 text-gray-700 font-medium">
            <Link to="/collections/men" className="hover:text-teal-600 transition">Men</Link>
            <Link to="/collections/women" className="hover:text-teal-600 transition">Women</Link>
            <Link to="/collections/kids" className="hover:text-teal-600 transition">Kids</Link>
          </nav>
          ...
          <div className="flex items-center space-x-4 text-gray-700">
            ...
            <Link to={user ? "/profile" : "/login"} className="hover:text-teal-600 transition">
              {user?.avatar ? (
            ...
            {/* Cart */}
            <button onClick={() => setDrawerOpen(!drawerOpen)} className="relative hover:text-teal-600 transition">
            ...
            <button className="md:hidden hover:text-teal-600 transition" onClick={() => setMobileOpen(!mobileOpen)}>
      ...
      {mobileOpen && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" onClick={() => setMobileOpen(false)} />}
      {drawerOpen && <div className="fixed inset-0 bg-black/40 z-40" onClick={() => setDrawerOpen(false)} />}

      <aside className={`fixed top-0 left-0 h-full w-64 bg-white z-50 shadow-xl transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <CartDrawer drawerOpen={drawerOpen} toggleDrawer={() => setDrawerOpen(false)} />
    </>
  );
};
